...
// By display name
const itemByName = gui.locator(i => i.displayName.includes('Session'));
...
test('clicking GUI item triggers callback', async ({ player }) => {
  ...
  const item = gui.locator(item => item.displayName.includes('guiItemInfo'));
...
test('navigate through pages', async ({ player }) => {
  ...
  // Check first page
  const firstItem = gui.locator(i => i.displayName.includes('Spawn'));
  ...
  const secondItem = gui.locator(i => i.displayName.includes('Arena'));
