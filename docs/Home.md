# Plugwright Test Framework Wiki

Welcome to the Plugwright Test Framework documentation!

## Getting Started

- [Getting Started](Getting-Started) - Installation and initial setup
- [Writing Tests](Writing-Tests) - Your first test and common patterns
- [Examples](Examples) - Real-world test examples

## Reference

- [Matchers Reference](Matchers-Reference) - Complete assertion API
- [GUI Testing](GUI-Testing) - Working with inventory GUIs
- [Test Filtering](Test-Filtering) - Run specific tests by file or name
- [Configuration](Configuration) - Gradle plugin options